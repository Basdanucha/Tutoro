    ...
    private fun getAndSaveFCMToken() {
        FirebaseMessaging.getInstance().token.addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                // Handle the case when getting the token is not successful
            }
        }
    }


    private fun saveTokenToFirebaseDatabase(token: String) {
        ...
        if (userId != null) {
            ...
            databaseReference.child(userId).child("fcmToken").setValue(token)
                .addOnSuccessListener {
                    println("Get token success $token")
                }
                .addOnFailureListener {
                    Toast.makeText(requireContext(), "Get token failed $token", Toast.LENGTH_SHORT)
                        .show()
                }
        }
    }
